...
from database import get_db, Distribution, FileInfo, Peer, FilePiece, db_operation
...
def get_tracker(db: AsyncSession = Depends(get_db)) -> P2PTracker:
    """
    获取P2P追踪器实例
    ...
    if tracker_instance is None:
        tracker_instance = P2PTracker(db)
...
async def upload_file(
    ...
    try:
        ...
        # 清除缓存（如果有）
        if 'clear_distributions_cache' in globals():
...
# 使用新的装饰器
@async_cached()
async def get_cached_distributions(db: AsyncSession):
    """获取带缓存的分发任务列表"""
    ...
    for dist in distributions:
        ...
        for file in files:
            ...
            piece_count_result = await db.execute(piece_count_stmt)
...
async def list_distributions(
    ...
    """
    try:
        return await get_cached_distributions(db)
    except Exception as e:
...
async def websocket_route(websocket: WebSocket, db: AsyncSession = Depends(get_db)):
    ...
    参数:
        ...
        db: 数据库会话
    """
    await websocket_endpoint(websocket, None, db)

@router.websocket("/ws/{client_id}")
async def websocket_route_with_client_id(websocket: WebSocket, client_id: str, db: AsyncSession = Depends(get_db)):
    """
    带客户端ID的WebSocket路由
